...
The main text and appendix tables report all task prompts, judgment criteria, per-run and task-level results, token details, and the price snapshot needed for cost computation. Raw session logs and execution scripts are not made public.
...
All result data (task-level and per-run pass rates, token details, cost computations) are reported in the main text and appendix tables; raw session logs and execution scripts are not made public.
